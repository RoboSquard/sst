...
(function($){
			...
			success: function(a){
				...
				if(error == ''){
					$.each(a, function(i,item){
						if(i < n){
							var html = [], q = item.link, u='<a href="'+href+'" class="link-user">'+id+'</a>', w='', x = '<a href="#" data-date="'+item.publishedDate+'" data-id="'+item.id+'">'+item.title+'</a>', y='', z='', zz='', m='', d = item.publishedDate, sq = q, st = item.title, s = '';
							switch(type)
							{
								case 'facebook':
									x = '';
								...
								z = item.text;
								var src = item.image;
								y = src ? '<a data-image_count='+i+' href="#"><img src="'+src+'" alt="" /></a>' : '' ;
								break;
							...
							if(type == 'twitter'){
								...
								s += share('','https://twitter.com/'+un+'/status/'+sq,opt.twitterId, i, item.hasNotes, item.scheduled);
							} else {
								s = share(st,sq,opt.twitterId,item.approve, i, item.hasNotes, item.scheduled);
							...
							var out = '<li rel="'+ob+'" class="dcsns-li dcsns-'+type+' dcsns-feed-'+fn+'" data-index="'+i+'" data-approved="'+item.approve+'">'+w+'<div class="inner">'+zz+'<span class="clear"></span></div>'+zintro+icon+'</li>', str = decodeURIComponent(opt.remove), rem = q;
							if(type == 'twitter'){
	...
	function nicetime(a,out,dates){
		...
		function share(st,sq,twitterId,status, index, hasNotes, scheduled){
			var s = '', sq = encodeURIComponent(sq), st = encodeURIComponent(st);

			//s += '<div class="" data-toggle="buttons"><label class="btn btn-block btn-sm btn-flat btn-warning active"><input type="radio" name="options" id="option1" autocomplete="off" checked><i class="fas fa-check"></i> CLAIMED</label><label class="btn btn-block btn-sm btn-flat btn-danger"><input type="radio" name="options" id="option2" autocomplete="off"><i class="fas fa-gift"></i> CLAIM NOW</label></div>';

			s = '<div class="btn btn-block btn-sm btn-flat btn-danger mb-2"><i class="fa-solid fa-arrow-down"></i> <small>REFER TWICE TO APPLY</small> <i class="fa-solid fa-arrow-down"></i> </div>';

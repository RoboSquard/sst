...
(function($){
			...
			this.defaults = {
				feeds: {
					...
					rss: {
						id: '',
						intro: 'Posted',
						out: 'intro,thumb,title,text,user,share,approve',
						url: 'rss.php',
						text: 'contentSnippet',
						icon: 'rss.png'
					},
	...
	function nicetime(a,out,dates){
		...
		function share(st,sq,twitterId,status, index, hasNotes, scheduled){
			...
			console.log(st+'===='+sq+'===='+twitterId+'===='+status);
			
			/*s += '<div class="actions" data-toggle="buttons"><label class="btn btn-block btn-sm btn-flat btn-warning active" data-val="no"><input hidden type="radio" name="options" id="option1" autocomplete="off" checked><i class="fa-solid fa-check"></i> Post Approved</label><label class="btn btn-block btn-sm btn-flat btn-success" data-val="yes"><input hidden type="radio" name="options" id="option2" autocomplete="off"><i class="fa-solid fa-plus"></i> Approve Post</label></div>';*/
			
			if(status=='yes')
			{
				s += '<div class="actions"><label class="btn btn-block btn-sm btn-flat btn-warning disabled" aria-disabled="true"><input hidden type="radio" name="options" id="option1" autocomplete="off" checked><i class="fa-solid fa-check"></i> Post Approved</label></div>';	
			}
			else
			{
				s += '<div style="display:none" class="actions"><label class="btn btn-block btn-sm btn-flat btn-success" data-val="yes"><input hidden type="radio" name="options" id="option2" autocomplete="off"><i class="fa-solid fa-plus"></i> Approve Post</label></div>';
			}
					
			s += '<a style="background-color: #9C27B0; background-image: linear-gradient(-45deg, #9C27B0 0%, #03A9F4 100%); padding-right:10px;padding-left:5px;padding-top:7px;padding-bottom:25px" class="editPost" href="javascript:;"><i class="fa-solid fa-plus"></i> EDIT POST</a>';
			s += '<a style="padding-right:10px;padding-left:5px;padding-top:7px;padding-bottom:25px;color:#343a40" href="javascript:void(0)">';
			if (hasNotes)
				s += '<i class="fa-solid fa-file-lines"></i> ';
			s += '<i class="fa-solid fa-clock '+(!scheduled? 'd-none': '')+'" href="#modal-scheduled" data-toggle="modal"></i> ';
			s += '<i class="fa-solid fa-trash" onclick="deletePost(this, '+index+')"></i>';
			s += '</a>';
			return s;
        }
})(jQuery);

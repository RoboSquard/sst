			...
			success: function(a){
				...
				if(error == ''){
					$.each(a, function(i,item){
						if(i < n){
							var html = [], q = item.link, u='<a href="'+href+'" class="link-user">'+id+'</a>', w='', x = '<a href="https://suite.social/s/jobs">'+item.title+'</a>', y='', z='', zz='', m='', d = item.publishedDate, sq = q, st = item.title, s = '';
							switch(type)
							{
								case 'facebook':
									x = '';
								...
								z = item.text;
								var src = item.image;
								y = src ? '<a href="https://suite.social/s/jobs"><img src="'+src+'" alt="" /></a>' : '' ;
								break;

...
// that before even requesting go.php).
////////////////////////////////
...
if ((_FF_FTR_MODE == 'full') && isset($_REQUEST['key']) && ($key_index = array_search($_REQUEST['key'], $options->api_keys)) !== false) {
	...
	$redirect = 'http://' . htmlspecialchars($host . $path) . '/go.php?url=' . urlencode($_qs_url);
	...
	$rewrite = filter_var($_GET['rewrite'], FILTER_SANITIZE_SPECIAL_CHARS);
	$redirect .= '&rewrite=' . urlencode($_REQUEST['rewrite']);
...
foreach ($items as $key => $item) {
	...
	if (($rewrite == 1) or ($rewrite == 2) or ($rewrite == 3)) {

		if (!class_exists('Spinter')) {
			class Spinter
			{
		...
		require_once('unik.class.php'); //new unik
	}

	if (($rewrite == 1) or ($rewrite == 3)) {
		$data = $item->get_title(); //new unik
		$unik = new unik(); //new unik
		$newsubject = $unik->spin($data); //new unik
		$spinter = new Spinter(); //new unik
		$feed_item_title = $spinter->process($newsubject); //new unik	
	} else {
	...
	if ($do_content_extraction) {
		...
		if (!$extract_result) {
			...
			//Rewrite Contents
			if (($rewrite == 1) or ($rewrite == 2)) {

				$data = $html; //new unik
				$unik = new unik; //new unik
				$newbody = $unik->spin($data); //new unik
				$spinter = new Spinter();
				$html = $spinter->process($newbody);
			}

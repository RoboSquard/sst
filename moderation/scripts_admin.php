      ...
      case result.isConfirmed: // Publish Now
        $.ajax({
          ...
          success: function (res) {
            ...
            self.innerHTML = '<i class="fa-solid fa-check"></i> Post Approved';
          }
        });
        break;
...
function addQueueItems(items, reRendring = false) {
  ...
  items.forEach((item) => {
	...
	itemEl = '<li' + (item.published ? ' class="done"' : '') + ' data-id="' + (itemIndex - 1) + '">' +
	  ...
	  '<small class="badge badge-success"><i class="far fa-clock"></i> ' + ordinalSuffixOf(pubDate.getDate()) + ' ' + month[pubDate.getMonth()] + '</small>' +
	  '<div class="tools">' +
  ...
  $.ajax({
	...
	success: function(res) {
	  ...
	  if (res.status) {
		...
		if (res.data.updatedRssUrlsAuto)
		  displayAutoRssUrls(res.data.updatedRssUrlsAuto, true);
	  }
	}
  })
}, 7000);
...
function updatePosts(posts) {
  posts.forEach((post) => {
	...
	if (post.scheduled) {
	  postEl.querySelector('.fa-clock').classList.remove('d-none');
	}

	const approvedBtn = postEl.querySelector('.actions');
	if (post.approve === 'yes') {
	  approvedBtn.innerHTML = '<label class="btn btn-block btn-sm btn-flat btn-warning disabled" aria-disabled="true"><input hidden="" type="radio" name="options" id="option1" autocomplete="off" checked=""><i class="fa-solid fa-check"></i> Post Approved</label>';
	  approvedBtn.style.display = 'block';
	} else {
	  approvedBtn.style.display = 'none';
	}
  });
  window.stream.isotope('layout');
}
...
const rssFields2 = document.querySelector('#rssFields2');

function addRssField2(self) {
  self.classList.add('d-none');
  self.parentNode.querySelector('.rm-field').classList.remove('d-none');
  let html = '<div class="input-group mb-2">' +
	'<input type="text" class="form-control new-field" name="urls[]">' +
	'<button type="button" class="add-field btn btn-success btn-sm btn-flat" onclick="addRssField2(this)"><i class="fa fa-plus"></i></button>' +
	'<button type="button" class="rm-field btn btn-danger btn-sm btn-flat d-none" onclick="rmRssField(this)"><i class="fa fa-minus"></i></button>' +
	'<button type="button" class="btn btn-default btn-sm btn-flat d-none"><i class="fa fa-clock"></i></button>' +
	'</div>';
  $(rssFields2).append(html);
}
const keywordfields = document.querySelector('#keywordfields');

function addkeywordfields(self) {
  self.classList.add('d-none');
  self.parentNode.querySelector('.rm-field').classList.remove('d-none');
  let html = '<div class="input-group mb-2">' +
	'<input type="text" class="form-control new-field" name="keywords[]">' +
	'<button type="button" class="add-field btn btn-success btn-sm btn-flat" onclick="addkeywordfields(this)"><i class="fa fa-plus"></i></button>' +
	'<button type="button" class="rm-field btn btn-danger btn-sm btn-flat d-none" onclick="rmRssField(this)"><i class="fa fa-minus"></i></button>' +
	'<button type="button" class="btn btn-default btn-sm btn-flat d-none"><i class="fa fa-clock"></i></button>' +
	'</div>';
  $(keywordfields).append(html);
}
...
// Import RSS
function importFromRss2(self) {
  var mydata = $('#importRssFormnew').serialize();

  $.ajax({
	url: '<?php echo BASE_URL; ?>cron.php?action=automatic_import_new_rss',
	type: 'POST',
	data: mydata,
	beforeSend: function() {
	  $(self).html('<span class="spinner-border spinner-border-sm d-inline-block"></span> Importing...');
	  $("#newRSS").val("");
	},
	success: function(res) {
	  console.log(res);
...
// Create Article
function createarticle(self) {
  if ($("#keyword").val()) {
	console.log($("#keyword").val());
	var mydata = $('#createarticles').serialize();
	$.ajax({
	  url: '<?php echo BASE_URL; ?>createarticles.php?action=create',
	  type: 'POST',
	  data: mydata,
	  beforeSend: function() {
		$(self).html('<span class="spinner-border spinner-border-sm d-inline-block"></span> Creating...');
	  },
	  success: function(res) {
		console.log(res);
...
// Schedule Keyword
function schedulekeyword(self) {
  var mydata = $('#createarticles').serialize();

  $.ajax({
	url: '<?php echo BASE_URL; ?>cron.php?action=automatic_schedule_keywords',
	type: 'POST',
	data: mydata,
	beforeSend: function() {
	  $(self).html('<span class="spinner-border spinner-border-sm d-inline-block"></span> Scheduling...');
	  $("#createarticles").get(0).reset(); //.val("");
	},
	success: function(res) {
	  res = JSON.parse(res);
	  $(self).html('Schedule');
	  if (res.status) {
		displayRssUrls(res.data);
		if (res.newdata) {
		  displayAutoRssUrls(res.newdata);
		}
		if (res.keywords) {
		  displaykeywords(res.keywords);
		}
	  } else {
		Swal.fire({
		  icon: 'error',
...
// Import RSS
function scheduleRss2(self) {
  var mydata = $('#importRssFormnew').serialize();

  $.ajax({
	url: '<?php echo BASE_URL; ?>cron.php?action=automatic_schedule_rss',
	type: 'POST',
	data: mydata,
	beforeSend: function() {
	  $(self).html('<span class="spinner-border spinner-border-sm d-inline-block"></span> Scheduling...');
...
// Delete RSS Auto
function deleteRssUrlauto(self) {
  let url = self.dataset.url;

  $.ajax({
	url: '<?php echo BASE_URL; ?>cron.php?action=delete_rss_url_auto',
	type: 'POST',
	data: {
	  url: url
	},
...
// Delete Keywords
function deletekeywords(self) {
  let url = self.dataset.url;

  $.ajax({
	url: '<?php echo BASE_URL; ?>cron.php?action=delete_keyword',
	type: 'POST',
	data: {
	  url: url
	},
...
function displayAutoRssUrls(mylist) {
  var urls = JSON.parse(mylist);
  var ops = ["Rewrite title & content", "Rewrite content", "Rewrite title", "Don't rewrite"];
  let html = '';
  urls.forEach((url) => {
	html += '<div class="input-group mb-2">' +
	  '<input type="text" class="form-control" name="urls[]" value="' + url.newRSS + '">' +
	  '<button type="button" class="rm-field btn btn-danger btn-sm btn-flat" onclick="deleteRssUrlauto(this)" data-url="' + url.newRSS + '"><i class="fa fa-minus"></i></button>' +
	  '<button type="button" class="btn btn-default btn-sm btn-flat"><i class="fa fa-clock"></i></button>' +
	  '<span class="form-group" style="font-size:8px;width:100%"> ' + url.links + '/' + (url.rewrite ? ops[parseInt(url.rewrite) - 1] : ops[3]) + '/' + url.sentences + '/' + (url.truncate ? url.truncate : "0") + '/' + url.imageapi + '/</span></div>';
  });
  $(rssFields2).html(html);
}

function displaykeywords(mylist) {
  var urls = JSON.parse(mylist);
  var ops = ["Rewrite title & content", "Rewrite content", "Rewrite title", "Don't rewrite"];
  let html = '';
  urls.forEach((url) => {
	html += '<div class="input-group mb-2">' +
	  '<input type="text" class="form-control" name="urls[]" value="' + url.keyword + '">' +
	  '<button type="button" class="rm-field btn btn-danger btn-sm btn-flat" onclick="deletekeywords(this)" data-url="' + url.keyword + '"><i class="fa fa-minus"></i></button>' +
	  '<button type="button" class="btn btn-default btn-sm btn-flat"><i class="fa fa-clock"></i></button>' +
	  '<span class="form-group" style="font-size:8px;width:100%"> ' + url.hashtags + '/' + url.language + '/' + url.sentences + '/' + (url.truncate ? url.truncate : "0") + '/' + url.imageapi + '/</span></div>';
  });
  $(keywordfields).html(html);
}

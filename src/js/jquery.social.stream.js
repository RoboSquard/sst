	...
	function getFeed(type,id,path,o,obj,opt,f1,f2,intro,feed,fn){
		...
		jQuery.ajax({
			...
			success: function(a){
				var error = '';
				if (a.item.length == 0) {
					$('.stream-container div').html('');
					$('.stream-container div').first().append('<div class="text-center"><br><br>No images found.<br><br><br></div>');
				}
				jQuery('.loader').hide();
				switch(type)
				{
				...
				if(error == ''){
					$.each(a, function(i,item){
						if(i < n){
							...
							var out = '<li rel="'+ob+'" class="dcsns-li dcsns-'+type+' dcsns-feed-'+fn+'">'+w+'<div class="inner">'+zz+'<span class="clear"></span><input type="text" class="form-control" value="'+this.image+'"><textarea class="form-control"><img src="'+this.image+'" ></textarea></div>'+zintro+icon+'</li>', str = decodeURIComponent(opt.remove), rem = q;
							if(type == 'twitter'){
